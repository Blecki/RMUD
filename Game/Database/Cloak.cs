...
*/
            ...
            //    now the bar is dark.
            Perform<MudObject, MudObject>("take").Do((actor, thing) => { GetObject("CloakOfDarkness.Bar").SetProperty("ambient light", LightingLevel.Dark); return SharpRuleEngine.PerformResult.Continue; });
            ...
            Perform<MudObject, MudObject, MudObject, RelativeLocations>("put")
                ...
                .Do((actor, item, container, relloc) => { GetObject("CloakOfDarkness.Bar").SetProperty("ambient light", LightingLevel.Bright); return SharpRuleEngine.PerformResult.Continue; });

            //Carry out dropping the cloak in the cloakroom:
            //    now the bar is lit.
            Perform<MudObject, MudObject>("drop")
                .When((actor, item) => actor.Location is Cloakroom)
                .Do((actor, item) => { GetObject("CloakOfDarkness.Bar").SetProperty("ambient light", LightingLevel.Bright); return SharpRuleEngine.PerformResult.Continue; });
